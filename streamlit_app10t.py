    ...
    if prompt := st.chat_input("Enter your prompt here..."):
        ...
        try:
            ...
            with st.expander("Groq Metrics"):
                ...
                with col1:
                    st.metric("Round Trip Time (RTT)", f"{rtt:.2f} ms")

                with col2:
                    st.metric("Speed", f"{speed:.2f} T/s")

                with col3:
                    st.metric("Inference Time", f"{inference_time:.2f} s")

                # Display tokens in a more readable way
                st.markdown(f"**Tokens:** Input: {input_tokens}, Output: {output_tokens}, Total: {total_tokens}")
